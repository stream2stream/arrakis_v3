...
SELECT id,bond_holder FROM CSVREAD('.\\java-api\\src\\main\\resources\\db-bonds-data.csv');


INSERT INTO security (id,isin,cusip,issuer_name,maturity_date,coupon,type,face_value,currency,status)
SELECT security_id,isin,cusip,issuer_name,maturity_date,coupon_percent,type,face_value,bond_currency,status FROM CSVREAD('.\\java-api\\src\\main\\resources\\db-bonds-data.csv');
...
SELECT trade_id,book_id,security_id,counter_party_id,trade_currency,trade_status,quantity,unit_price,trade_type,trade_date,settlement_date FROM CSVREAD('.\\java-api\\src\\main\\resources\\db-bonds-data.csv');
